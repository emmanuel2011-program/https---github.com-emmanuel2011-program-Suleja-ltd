...
export default function InvestmentTable({ initialInvestments, isAdmin }: { initialInvestments: any[], isAdmin: boolean }) {
  ...
  return (
    /* FIX 1: added 'pb-32'. This creates extra space at the bottom 
       of the list so the footer doesn't overlap the last item.
    */
    <div className="w-full pb-32 md:pb-10"> 
      <div className="bg-white rounded-xl shadow-md border border-gray-200">
        
        {/* FIX 2: Ensure 'overflow-x-auto' is handled correctly. 
            Removed 'overflow-hidden' from parent to prevent row clipping.
        */}
              ...
              {investments.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 md:px-6 py-4">
                      <p className="font-black text-gray-900 text-sm md:text-lg">₦{Math.round(inv.total_due).toLocaleString()}</p>
                      {inv.withdrawn_to_date > 0 && (
                        <p className="text-[8px] text-red-500 font-bold uppercase italic">-₦{inv.withdrawn_to_date.toLocaleString()} Withdrawn</p>
                      )}
                    </td>

                    <td className="px-4 md:px-6 py-4 bg-green-50/30">
                      <p className="text-sm font-black text-green-700">₦{Math.round(inv.yield_in_naira).toLocaleString()}</p>
